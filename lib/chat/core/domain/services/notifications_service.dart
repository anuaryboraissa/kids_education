  ...
  onNotificationClicked(RemoteMessage event) {
    ...
    if (conversationId != null){
      if ('true' == event.data['joinCall']) {
         Navigator.of(navigatorKey.currentContext!).pushAndRemoveUntil(MaterialPageRoute(builder: (context) => CallScreen(conversationId: conversationId),), (route) => false);
        // Navigator.of(navigatorKey.currentContext!).pushNamedAndRemoveUntil(ScreenRoutes.call, (route) => route.isFirst, arguments: CallScreenArgs(conversationId: conversationId));
                 ...
                 Navigator.of(navigatorKey.currentContext!).pushAndRemoveUntil(MaterialPageRoute(builder: (context) => RealtimeChatScreen(conversationId: conversationId, uidForDirectConversation: uidForDirectConversation),), (route) => false);
