...
class AddGroupParticipantsScreen extends StatefulWidget {
 ...
 final String conversationId;
  const AddGroupParticipantsScreen({super.key, required this.conversationId});
...
class _AddGroupParticipantsScreenState extends State<AddGroupParticipantsScreen> {
  ...
  void didChangeDependencies() {
    ...
    initialized = true;
    controller = AddParticipantsController(conversationId: widget.conversationId);
    ...
    return MyScaffold(
      ...
      body: ValueListenableBuilder(
          ...
          builder: (context, users, _) {
            final selectCount = users.entries.where((entry) => entry.value["bool"]).length;
                      ...
                      children: [
                        ...
                        ...filteredUsers.mapIndexed((i, e) {
                          return Padding(
                              ...
                              child: InkWell(
                                onTap: () {
                                  controller.selectUserChanged((e.value['user_public'] as UserPublic).uid, !e.value["bool"]);
                                },
                                child: Ink(
                                  child: Column(
                                    children: [
                                          ...
                                          Expanded(
                                            child: Row(
                                              ...
                                              children: [
                                                ...
                                                Text((e.value['user_public'] as UserPublic).fullName, style: TextStyle(color: Colors.white, fontSize: 17, fontWeight: e.value["bool"] ? FontWeight.w700 : FontWeight.w600)),
                                              ],
                                            ),
                                          ),
                                          Container(
  ...
  bool checkIfMatchesFilter(MapEntry<String, Map<String,dynamic>> element) {
    String unformatted (String value) {
      return value.toString().toLowerCase().replaceAll(' ', '');
    }
    return unformatted((element.value['user_public'] as UserPublic).fullName).contains(unformatted(searchController.text));
  }
}
