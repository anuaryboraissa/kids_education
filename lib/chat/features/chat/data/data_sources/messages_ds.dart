...
import 'package:mathgame/chat/features/chat/data/data_sources/typing_ds.dart';
...
import '../../domain/entities/sending_text_message_entity.dart';
...
class MessagesDS {
  ...
  Stream<List<Message>> messagesStream({required String conversationId, int? limit, void Function(List<Message> newReceivedMessageList)? onNewReceivedMessage}) {
    ...
    sub = query.snapshots(includeMetadataChanges: true).listen((event) {
  ...
  Stream<Conversation> conversationStream({required String conversationId}) {
    ...
    final subscription = conversationRef.snapshots().listen((snapshot) async {
