...
class RealtimeChatPageController {
  ...
  RealtimeChatPageController({required this.currentContext, required this.conversationId, required this.scrollToBottom, required this.scrollController}) {
    detailedConversationController = DetailedConversationController(
      ...
      onLoad: () {
        if (detailedConversationController.last!.isGroup && detailedConversationController.last!.users.length == 1) {
          ...
          // );
          Navigator.of(currentContext).push(MaterialPageRoute(builder: (context) => CreateGroupOrEditTitleScreen(editExistingConversationId: conversationId),));
        }
      }
    );
  ...
  bool get _scrollIsCloseToTheBottom => scrollController.position.maxScrollExtent == 0 || (scrollController.position.pixels > 0 && (scrollController.position.maxScrollExtent - scrollController.position.pixels < (MediaQuery.of(currentContext).size.height * .1)));
}
